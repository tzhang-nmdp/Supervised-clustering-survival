...
survival_summary_model<-function(input_matrix, marker_index, km_name)
    ...
    rownames(imp_dict_494_all)<-colnames_tmp[which(colnames_tmp %nin% c("intxsurv", "dead"))]
    cidx_494_all<-c()
    cidx_134_all<-c()
    ...
    for (i in 1:100)
        ...
        cidx_494_all<-rbind(cidx_494_all,c_index) 
        }
    print(modRFSRC_494_all)

    # Random forest survival model performance check 
    cindex_matrix[,1]<-quantile(cidx_494_all, probs=c(0.05, 0.95))

    # plot the feature importance
    imp_dict_494_all[,2]<-saplly(imp_dict_494_all[,2:dim(imp_dict_494_all)[2]],mean)
    imp_dict_494_all<-imp_dict_494_all[,1:2]
    imp_dict_494_all$V3<-'mutational'
    colnames(imp_dict_494_all)<-c('feature_importance','feature_name','feature_category')
    ...
    cindex_matrix[,2]<-quantile(cidx_494_all, probs=c(0.05, 0.95))
