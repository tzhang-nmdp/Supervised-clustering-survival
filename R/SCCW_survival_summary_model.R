...
survival_summary_model<-function(input_matrix,kk_x,delta, km_name)
{
    ...
    imp_dict_all<-data.frame(matrix(ncol = 1, nrow = dim(input_matrix)[2]-2))
    colnames_tmp<-colnames(input_matrix)[order(colnames(input_matrix))]
    imp_dict_all[,1]<-colnames_tmp[which(colnames_tmp %nin% c("intxsurv", "dead"))]
    cidx_rsf_all<-c()
    ...
    for (i in 1:100)
        ...
        modRFSRC_all <- rfsrc(Surv(intxsurv, dead) ~ ., data=new.data,block.size=1,err.block=1,  na.action = "na.impute",importance = TRUE)
        imp_all<-as.data.frame(modRFSRC_all$importance)
        imp_all[,2]<-rownames(imp_all)
        imp_all<-imp_all[order(rownames(imp_all)),]
        imp_dict_all<-cbind(imp_dict_all,imp_all[,1]) 
        c_index<-c(1-median(modRFSRC_all$err.rate))
        cidx_rsf_all<-rbind(cidx_rsf_all,c_index) 
        }
    print(modRFSRC_all)

    # Random forest survival model performance check 
    cindex_matrix[,1]<-c(median(cidx_rsf_all),quantile(cidx_rsf_all, probs=c(0.05, 0.95)))

    # plot the feature importance
    imp_dict_all[,2]<-apply(imp_dict_all[,2:dim(imp_dict_all)[2]],1,mean)
    imp_dict_all<-imp_dict_all[,1:2]
    imp_dict_all$V3<-'mutational'
    colnames(imp_dict_all)<-c('feature_name','feature_importance','feature_category')
    imp_dict_all$feature_name<-as.character(imp_dict_all$feature_name)
    for (i in 1:dim(imp_dict_all)[1])
        {
    if  (grepl('_id',imp_dict_all[i,2]))
        {
        imp_dict_all[i,3]<-'clustering'
    }
        else if  ( imp_dict_all[i,2] %in% c('ipssr','HMA', 'CHEMO','mdstype'))
        {
        imp_dict_all[i,3]<-'clinical'
    }    
    }
    imp_dict_all$feature_importance_rev<-sapply(imp_dict_all$feature_importance, function(x) { x*(-1)})
    #imp_dict_all[imp_dict_all$feature_name=='km_id_com', 'feature_name']<-'cluster_common_variant'
    #imp_dict_all[imp_dict_all$feature_name=='km_id', 'feature_name']<-'cluster_rare_variant'
    #imp_dict_all[imp_dict_all$feature_name=='rc_id', 'feature_name']<-'cluster_recurrent_variant'
    options(repr.plot.width=20, repr.plot.height=10)
    p<-ggplot(imp_dict_all,aes(x=reorder(feature_name,feature_importance_rev),y=feature_importance,fill=feature_category)) + geom_bar(colour="black",stat="identity", width=.5, position = "stack",alpha=1)+theme(axis.text.x=element_text(angle=90,size=15),axis.text.y=element_text(size=15),axis.title.x=element_text(size=15),axis.title.y=element_text(size=15),legend.text=element_text(size=15))
    ...
    for (i in 1:100)
        ...
        new.data <- tempSubset_fs_tmp[index,]   
        modRFSRC_all <- coxph(Surv(new.data$intxsurv, new.data$dead) ~ ., data=new.data)
        c_index<-modRFSRC_all$concordance[6]
        cidx_cox_all<-rbind(cidx_cox_all,c_index) 
        }
    cindex_matrix[,2]<-c(median(cidx_cox_all),quantile(cidx_cox_all, probs=c(0.05, 0.95)))
    ...
    cidx_xgb_all<-c(cidx_xgb_all,concordance_index(tempSubset_fs_tmp$intxsurv,1-risk_scores))
        }

    cindex_matrix[,3]<-c(median(cidx_xgb_all),quantile(cidx_xgb_all, probs=c(0.05, 0.95)))
