...
write.table(km_mc_cluster_id_score_matrix[[1]],paste(outdir,"/",km_name,"_delta_",delta,"_K_",kk_x,"_oob_",oob,"marker_cutoff",marker_cutoff_metrics,"_replication.sig_validation.csv",sep=""), sep="\t", quote=F,col.names=T,row.names=F) 
            ...
            save(km_mc_cluster_id_score_matrix,input_matrix,km_mc_model, stat_go_weight_vector, marker_cutoff_metrics, km_name, kk_x, delta, oob,file=paste(outdir,"/",km_name,"_delta_",delta,"_K_",kk_x,"_oob_",oob,"marker_cutoff",marker_cutoff_metrics,"_.val_model.RData",sep="")) 
...
supervised_clustering_val<- function(input_matrix, marker_index, km_mc_model, stat_go_weight_vector, marker_cutoff_metrics, km_name, kk_x_max, delta_max) 
    ...
    # load training model settings for validation 
    load(paste(outdir,"/",km_name_t,"marker_cutoff",marker_cutoff_metrics,"_tmp.sig_parameter.RData",sep=""))
    kk_x<-kk_x_max
    delta<-delta_max
    load(paste(outdir,"/",km_name_t,"_delta_",delta,"_K_",kk_x,"_oob_",oob,"marker_cutoff",marker_cutoff_metrics,"_val_model.RData",sep="")) 
    ...
    print(paste("# predict cluster id and calculate the score for multiple phenotypes", Sys.time(),sep=" "))  
    km_mc_cluster_id_score_matrix <-validation_predict(km_mc_model, input_matrix, stat_go_weight_vector, marker_index, marker_cutoff_metrics, kk_x, num_oob)   
    write.table(km_mc_cluster_id_score_matrix[[1]],paste(outdir,"/",km_name,"marker_cutoff",marker_cutoff_metrics,"_replication.sig_validation.csv",sep=""), sep="\t", quote=F,col.names=T,row.names=F) 
    #print(km_mc_cluster_id_score_matrix)
